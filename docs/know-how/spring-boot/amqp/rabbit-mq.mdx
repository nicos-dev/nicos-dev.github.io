...
You have to connect 2 services with each other via Exchange **usu.user-exchange**. There'll be 2 Queues **usu.user-service** and **usu.cache-service**
...
<Highlight color="#4287f5">Cache Service</Highlight> &#10132; Message with <Highlight color="#11d1ce">usu.user.sync-request</Highlight> &#10132; <Highlight color="#f54260">usu.user-exchange</Highlight>
...
<Highlight color="#4287f5">User Service</Highlight> &#10132; Message with <Highlight color="#11d1ce">usu.user.sync-response</Highlight> &#10132; <Highlight color="#f54260">usu.user-exchange</Highlight>
...
<Highlight color="#4287f5">User Service</Highlight> &#10132; Message with <Highlight color="#11d1ce">usu.user.created</Highlight> &#10132; <Highlight color="#f54260">usu.user-exchange</Highlight>
...
<Highlight color="#4287f5">User Service</Highlight> &#10132; Message with <Highlight color="#11d1ce">usu.user.deleted</Highlight> &#10132; <Highlight color="#f54260">usu.user-exchange</Highlight>
...
<Highlight color="#4287f5">User Service</Highlight> &#10132; Message with <Highlight color="#11d1ce">usu.user.updated</Highlight> &#10132; <Highlight color="#f54260">usu.user-exchange</Highlight>
...
```kotlin
import com.usu.usermanagement.config.amqp.RabbitMqConfig
...
class UserRabbitMqSenderServiceImp: UserRabbitMqSenderService {
    ...
    override fun requestUsers() {
        rabbitMqTemplate.send(
            "usu.user-exchange", // Exchange
            "usu.user.sync-request", // Routing Key
...
To receive any messages you have to declare bindings. Because Messages are send to <Highlight color="#f54260">usu.user-exchange</Highlight>
...
class UserRabbitMqReceiver {
    ...
    @RabbitListener(
        bindings = [
            QueueBinding(
                value = Queue(value = "usu.cache-service", durable = "true"),
                exchange = Exchange(value = "usu.user-exchange", type = ExchangeTypes.TOPIC),
                key = [
                    "usu.user.sync-response",
                    "usu.user.created",
                    "usu.user.updated",
                    "usu.user.deleted"
                ]
    ...
    fun messageArrived(@Payload value: String, @Header(AmqpHeaders.RECEIVED_ROUTING_KEY) key: String) {
        when (key) {
            "usu.user.sync-response" -> syncUsers(value)
            "usu.user.created" -> userCreated(value)
            "usu.user.updated" -> userUpdated(value)
            "usu.user.deleted" -> userDeleted(value)
        }
    }
